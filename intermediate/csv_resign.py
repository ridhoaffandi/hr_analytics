...
select
    ...
    status
from daftar_pegawai
where status like '%Resign%'
order by termination asc
"""
...
def upload_csv_to_postgres(csv_path, table_name, db_config):
    ...
    insert_query = f"""
       INSERT INTO resign (nik,company,hire,termination,status)
            VALUES %s
    """
    ...
    cursor.execute("TRUNCATE TABLE resign")
